class TwoVarianceController {
  constructor() {
    this.model = new TwoVarianceModel();
    this.view = new TwoVarianceView();
  ...
  initialize() {
    this.view.twovariannceFormEl.reset(); // Reseta os campos do formulário sempre que a página carregar.
    ...
    this.view.twovarianceFormEl.addEventListener("submit", (event) => {
  ...
  listenSubgroupsChanges() {
    [...this.view.subgroupsCheckboxesEl.children].forEach(el => {
      el.children[0].addEventListener("change", () => {
        this.validateForm(); // Chama o método que verifica se o formulário pode ser enviado.
        // TODO: verificar se existem mais de 2 opções selecionadas
      });
    });
  }
  ...
  validateForm() {
    ...
    // TODO: verificar se exitem mais de 2 checkboxes selecionadas (criar método).
  ...
  getFormValues() {
    ...
    this.model.submitForm(formData, (response) => {
        ...
        let url = "twovariancegraphs.html";
